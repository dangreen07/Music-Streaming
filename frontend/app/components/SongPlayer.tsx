...
export default function SongPlayer({server_url, initial_song_duration}: {server_url: string, initial_song_duration: number}) {
    ...
    const [currentSong, setCurrentSong] = useState<string>("Crab Rave.wav");

    async function GetAudio(sample_number: number = 0) {
    const response = await fetch(server_url + "/sample/" + encodeURI(currentSong) + "/" + sample_number);
    ...
    const interval = setInterval(async () => {
        ...
        if (currentTime >= initial_song_duration) {
    ...
    return (
        ...
        <div className="flex flex-col items-center justify-center gap-2 w-1/2">
          ...
          <div className="flex w-full items-center gap-2">
            ...
            <progress className="progress w-full" value={currentTime} max={initial_song_duration}></progress>
            <span id="song-duration" className="text-white text-md">{formatTime(initial_song_duration)}</span>
          </div>
        </div>
